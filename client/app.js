...
function onPageLoad() {
    ...
    var url = "http://127.0.0.1:5000/get_buildyear"; 
    $.get(url, function (data, status) {
        console.log("got response for get_buildyear request");
        if (data) {
            var by = data.by;
            var uiBuildYear = document.getElementById("uiBy");
            $('#uiBy').empty();
            for (var i in by) {
                var opt = new Option(by[i]);
                // add BuildYear into dropdown
                $('#uiBy').append(opt);
            }
        }
    })
